...
export def ps [
    ...
    let ps_out = (/usr/bin/ps -ewwo pid,ppid,%cpu,user,label,rss=mem,vsz,etimes,cmd | lines | parse -r '(?P<pid>\d+)\s+(?P<ppid>\d+)\s+(?P<cpu>[0-9.%]+)\s+(?P<user>\w+)\s+(?P<selinux>[^\s]+)\s+(?P<memory>\d+)\s+(?P<virtual>\d+)\s+(?P<created>[^\s]+)\s+(?P<command>[^\s]+).*' | into decimal pid created memory virtual | update memory { |r| $r.memory * 1024 } | update virtual { |r| $r.virtual * 1024 } | update created {|r| ((date now | date format "%s" | into decimal) - $r.created) | into string | into datetime -z l })
