...
def extract_url(text, pos, url_prefix):
    ...
    while True:
        ...
        try:
            if (text[pos] == " " and text[pos + 1] == SEPARATOR) or text[
                pos
            ] == "\n":
                count = 1 if text[pos] == "\n" else 0
                old_pos = pos
        ...
        if (
            prefix_pos < len(url_prefix) - 1
            and text[pos] != url_prefix[prefix_pos]
        ):
            break
...
def mark(text, args, Mark, extra_cli_args, *a):
    ...
    while start_pos < len(text):
        ...
        if not url:
            start_pos, end_pos, url = extract_url(
                text, start_pos, "http://"
            )
        if url:
...
def handle_result(args, data, target_window_id, boss, extra_cli_args, *a):
    ...
    for match, match_data in zip(matches, groupdicts):
        boss.launch(
            "--type=overlay", "kitty", "+kitten", "icat", "--hold", match
        )
